...
public interface StatisticsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query(value = "select new ru.practicum.statistics.models.dto.ViewStats(e.app, e.uri, count(e.ip)) " +
    ...
    @Query(value = "select new ru.practicum.statistics.models.dto.ViewStats(e.app, e.uri, count(distinct e.ip)) " +
            "from EndpointHit as e " +
            "where e.timestamp between ?1 and ?2 " +
            "group by e.uri, e.app")
